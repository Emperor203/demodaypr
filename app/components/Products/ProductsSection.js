...
import { CATEGORY_LABELS_MAP } from "../../lib/labels";
...
const normalizeExternalProduct = (item) => {
  ...
  return {
    ...
    description:
      typeof item.description === "string" && item.description.trim() ? item.description : "Описание отсутствует.",
    category: typeof item.category === "string" && item.category.trim() ? item.category : "без категории",
    image,
    rating: {
...
export default function ProductsSection() {
  ...
  return (
    <div className="container-custom w-full bg-[var(--background)] py-4 text-[var(--foreground)] sm:py-6 md:py-10">
      <Header />
      <div className="mb-6">
        <p className="text-sm text-[var(--color-muted)]">Главная / Товары</p>
        <h1 className="mt-1 text-3xl font-black uppercase tracking-tight sm:text-4xl">Товары</h1>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <aside className="space-y-8 lg:col-span-3">
          ...
          <div>
            <h3 className="text-sm font-bold uppercase mb-3">Размер</h3>
          ...
          <div>
            <h3 className="text-sm font-bold uppercase mb-3">Наличие</h3>
          ...
          <div>
            <h3 className="text-sm font-bold uppercase mb-3">Категория</h3>
            <div className="flex flex-wrap gap-2">
              {categories.map((category) => (
                ...
                >
                  {CATEGORY_LABELS_MAP[category] || category}
                </button>
              ))}
            </div>
          </div>
        ...
        <section className="lg:col-span-9">
          <div className="mb-6 flex flex-col gap-3 md:flex-row md:items-center">
            ...
            />
            <p className="text-sm text-[var(--color-muted)]">Найдено: {filteredProducts.length}</p>
          </div>

          {loading && <p className="uppercase text-sm">Загрузка товаров...</p>}
          {error && <p className="text-red-500 text-sm">{error}</p>}

          {!loading && filteredProducts.length === 0 && (
            <p className="text-sm text-[var(--color-muted)]">По выбранным фильтрам товары не найдены.</p>
          )}
              ...
              {totalPages > 1 && (
                <div className="mt-8 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                  <p className="text-lg font-medium text-[var(--foreground)]">
                    {rangeStart}-{rangeEnd} из {filteredProducts.length} товаров
                  </p>
